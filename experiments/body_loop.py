...
ap.add_argument("--hill-force-scale", type=float, default=1.0, help="--muscles hill: ONE multiplier on every Hill muscle's peak isometric force F_max (and so on its passive force, which MuJoCo's muscle takes as a fraction fpmax of F_max); 1.0 = FlyMimic as fitted (default). a CHOICE, labelled, inside the span between two sources that disagree (ledger row 27; nate: 'disagreement in the literature leaves room for us to play'). the arithmetic, at the tibia flexor (the one joint Azevedo measured): FlyMimic's LFTibia_flex F_max 68.1 uN x its moment arm about the tibia hinge 15.2 um = 1.04 nN.m at full activation, optimal length, isometric (a tip force of 1.04 nN.m / 417 um = 2.5 uN). Azevedo et al. 2020 (eLife 9:e56754) measured force at the tibia tip on a probe, lever 417 +- 7 um: one FAST spike ~10 uN x 417 um = 4.2 nN.m (the twitch's peak, ~21 ms after the spike); the whole joint at most ~90-100 uN x 417 um = 38-42 nN.m (fig 1F, all units). the torque path's --gain 42 is that whole-joint number: a spike at f_w 1 adds K / --sat 10 = 0.1 of it at the kernel's peak, 4.2 nN.m per fast spike, ten of them summed reach 42. so FlyMimic vs Azevedo is x4 per fast spike and x36-40 maximum to maximum; FlyMimic's full activation is its tetanus, so the like-for-like span is 1 to ~40 (4 = one real fast spike's peak on the whole FlyMimic flexor). in --hill-drive twitch a fast spike's excitation peaks at 0.1, so a scale of 40 gives the torque path's 4.2 nN.m per spike (less force-length / velocity); in --hill-drive spike a fast spike saturates the muscle for ~1 ms, an impulse of ~1.4 x 1.04 x scale nN.m.ms against the twitch's 4.2 x 35 = 147")
...
if args.muscles == "hill":   # (09-23, ledger row 27) FlyMimic's Hill muscles on six legs, experiments/hill_legs.py; added to the spec before the world compiles
    ...
    MUS = dict(list=hill_legs.build(fly.mjcf_root, jm, lambda x: f"{x.parent.name}->{x.child.name}:{x.axis.value}", neutral_of, roles, _tol, arm_rule=args.hill_arm, force_scale=args.hill_force_scale, print=print))
...
if MUS is not None:   # (--muscles hill) per role and side of the joint, pooled over legs, after the warm-up: mean activation, mean and p99 torque (force x |gear|)
    ...
    print(f"--muscles hill, per leg after the warm-up (--hill-force-scale {args.hill_force_scale:g}): p99 of each muscle's torque (tension x |arm|), the largest per leg, nN.m: " + " ".join(f"{l} {max(np.percentile(_tq[:, k], 99) for k, x in enumerate(MUS['list']) if x['leg'] == l):.2f}" for l in LEG6 if any(x["leg"] == l for x in MUS["list"])))
