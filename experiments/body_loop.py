...
for ms in range(n_ms):
    ...
    if adh: sim.set_leg_adhesion_states("nmf", np.array([grip[l][ms] > 0.05 for l in LEG6]))
...
np.savez_compressed(args.out + ".npz", thorax=P, quat=Q, leg_force=FL, knee=KA, args=np.array(str(vars(args))))
