...
def run(integrate, rev, kick=True):
...
def psp(integrate, rev): a, p = run(integrate, rev); b, _ = run(integrate, rev, kick=False); return (a.astype(np.float64) - b), a, p
