...
**the release (16:04 PDT, 09-23):** the lift side is awake under row 28 and the hold side never lets go; a step is a release. the 13B lineage
(the 13A cells' opponents in the file and in Syed 2026) and every inhibitor of the hold side are logged on the listening rig with a deaf
...
| 1 | 09-22 | claw labels reported both ways (as printed / swapped). PINNED 09-23 (`claw_and_13A.md`): SNpp50 = flexion-tuned, SNpp51 = extension-tuned, i.e. `50flex`, at 85-90 % from the file's wiring against Lee 2025 and Syed 2026; `50ext` is positive feedback and a control only | no paper names the types | an axon match against Mamiya 2018 / NBLAST against FANC's claw axons, or a recording |
...
| 27 | 09-23 | the muscle model: Azevedo's twitch kernel summed linearly per spike, clipped at +-60 nN.m, no fusion, no length-tension. TESTED 09-23 (`--muscles hill`, FlyMimic's Hill muscles built onto six legs by rule): on the sourced springs they cannot move a leg (1-10 nN.m peak against Azevedo's ~42); on FlyMimic's soft springs they move the legs with a 5 ms lag and cannot stand. the open question is force scale, not fusion | the fly's twitch is sourced; its summation at 50-100 Hz is not, and a real fly's legs on this body walk at 11 Hz by position control where the muscle path cannot carry more than ~8 | a measured force-frequency curve for a fly leg muscle, or FlyMimic's Hill muscles on all six legs |
| 28 | 09-23 (15:35) | **item 7, the defensible form:** the input synapses onto the four excitatory premotor types that lock to a real step's swing (IN03A004, IN17A028, IN21A022, IN21A010; 30 cells) scaled x2 / x5 / x10 through the size path, cord then body, under DNg100 | the arc is present and phase-correct in the file and fires at a hundredth of a step's rate; the hold side is wired 6-11x beyond its size where these are wired to size; no measurement of these cells' synaptic weights exists | a measurement; every number under this row is a fit, checked against the listening rig's phase (a swing-locked whisper should become a swing-locked burst), the deaf twin and the puppet's numbers |
| 29 | 09-23 (16:04) | a scale on FlyMimic's muscle forces (`--hill-force-scale`), chosen inside the span between FlyMimic's fitted forces (1x) and Azevedo 2020's measurement (~4-40x, the ratio to be reconciled), with the springs beside it | two sources disagree and the body stands on neither; nate: "disagreement in the literature leaves room for us to play" | a fly leg muscle's force-frequency curve; until then every Hill arm carries its scale in its name. RUN 09-23: the ratio is 4x per spike and 36-40x max-to-max; at x10 on the sourced springs the Hill muscles carry an 11 Hz step with zero lag and he stands 17 s then flips; x6-7 stand the run; no arm makes a tripod; the crouch and the short swings remain |
| 30 | 09-23 (16:09) | the compound arm: the inputs onto the four swing-locked releasers of the hold side (IN14A008, IN13B013, IN13B006, IN13B004) scaled x3 / x5 / x10 beside the lift side's excitors at x3 (row 28) | both sides of a step are present in the file, phase-correct and starved, and the lift alone drops him | a measurement of either side's weights; every number under this row is a fit, checked on the listening rig's phase, the deaf twin and the puppet |
| 31 | 09-23 (16:16) | **the solver** (`experiments/solver/`, `docs/SOLVER.md`, being built): an optimiser over ~16 labelled gains on named cell sets (rows 28-30's sets, the hold side, the command's inhibitors, the subnet, the levators, the plateau strength, DNg100's rate), objective = the replay's numbers (standing, lift rate 5-11 Hz, tripod antiphase, forward speed) with a physiology penalty, two seeds, cord prefilter | nate: "another comparison point, like the puppet or the live-fly motion"; and (20:45) "isn't the fundamental circuitry still the fly?": yes. nine scalars on named sets cannot make the wiring do what it lacks the cells for; what they find is the fly's circuitry with its gains set, as Pugliese's, Lappalainen's and Kakaria's are | a result under this row, reported with its vector: which sets it leaned on, how far from the file's weights, checked on the listening rig (the phase is still the senses') and by the reads (a slide, a hop or a held leg is not a step). the earlier wording, "a puppet by construction, never a result", overstated it and is withdrawn here |
