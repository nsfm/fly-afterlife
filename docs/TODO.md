...
| ocelli (46 interneurons) | silent | **the photoreceptors are outside the imaged volume** (16:30: nothing entering by the ocellar nerve touches OCG / OCC; their input is all typed brain cells) | [x] built and run 17:35: breaks nothing, hint of faster (6.3-7.2 s vs 6.9-11.2), no measurable effect on open ground; DEFAULT ON. next: a run that crosses shade with the ocellar cells logged | light-level and horizon reflexes; whether the ocellar descending path (DNx02? four cells in the ocellar nerve, near-zero input in the volume, 10,000 outputs each onto AN06B025 / GNG) does anything |
