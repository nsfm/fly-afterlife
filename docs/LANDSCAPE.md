...
| **flyproject.io browser sandbox** (served JS, no licence served; `docs/FLYPROJECT_WEB.md`, 09-19) | the same BANC brain byte for byte (158,262 cells, 3,126,254 edges); `brain_worker.js` a line-for-line port of the jar | a browser arena with placeable stimuli | the stimuli are `atan2` on prop coordinates labelled with receptor names never consulted | L-R of all DNs, 98 % of it `Math.random()`; the brain steers only in branch 10 of a 10-branch if/else | no: the jar's [brain]/[model] labels are gone, a "memory" prints "synapses updated" with no mushroom body loaded, the male is her rig tinted darker | zeroing every synapse leaves the turn statistics unchanged (SD 0.0580 both ways); the 429-cell escape circuit is real (with a hand-inserted x50 boost); the optic lobe, half the network, fires 0.000 spikes per step; worth stealing: the live-brain panel with per-neuron baseline subtraction, the loom split (theta vs dtheta/dt), count-then-scatter Poisson |
