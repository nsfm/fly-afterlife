...
for w in old new; do
  ...
  for s in 3 4; do
    ...
    uv run python world/pair.py --seconds 30 --seed $s --deterministic --wsyn-m $WM --wsyn-f $WF --bristle hold --legmn all --no-floor --pace-k 4 --walk-dn DNp09 --pace fixed --out "world/oracle/port_${w}_s$s.npz" > "world/oracle/port_${w}_s$s.log" 2>&1 || echo "port $w s$s FAILED"
  done
done
uv run python - <<'PY'
