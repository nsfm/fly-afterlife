...
for w in old new; do
  ...
  for s in 3 4; do
    ...
    uv run python world/pair.py --seconds 30 --seed $s --deterministic --wsyn-m $WM --wsyn-f $WF --walk 100 --antennae wide --wind off --thermo off --ocelli off --wind-sated off --std off --out "world/oracle/v2port_${w}_s$s.npz" > "world/oracle/v2port_${w}_s$s.log" 2>&1 || echo "v2 port $w s$s FAILED"
  done
done
uv run python - <<'PY'
