...
for i in 000 001 002 003 004 005 006 007 008 009; do
  uv run python world/loop.py --mode spin --model flow/0000/$i --seed 0 --seconds 12 --out world/ens/spin_m$i.npz > world/ens/spin_m$i.log 2>&1
  echo "model $i done $(date +%H:%M)"
done
echo "spin done $(date +%H:%M)"
