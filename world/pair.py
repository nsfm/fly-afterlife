...
_HOOKS = [h for h in (globals().get(n_) for n_ in ("_hook_compass", "_hook_cells", "_hook")) if h is not None]   # each is defined only under its own flag   # 09-19 18:05: ALL that apply, in this order. before, the slot took one hook and --log-types evicted the compass (no goal yield, no wind goal) in any run that logged cells
