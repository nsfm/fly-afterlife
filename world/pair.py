...
from fly_afterlife.receptors import Registry, ReceptorClass, Hold, Adapting, Scaled, TapBurst, GaitLeg
REG = Registry()
...
leg_gain, asym_side = reflex_gain(M, RM, TACT_M, CH, SPF, drive_hz=(150.0 if args.bristle == "hold" else Adapting().fraction * Adapting().plateau_hz)); print(f"touch reflex: leg-MN asymmetry (R-L)/(R+L) with left bristles {asym_side['L']:+.3f}, right {asym_side['R']:+.3f} -> gain {leg_gain:.0f} deg per unit asymmetry")
