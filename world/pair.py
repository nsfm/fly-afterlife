...
if args.floor:
    ...
    _fl = tonic_floor(M, REG); _flc = np.unique(np.concatenate([rc.cells for rc in _fl])); M.driven[_flc] = True; M._driven_idx = np.flatnonzero(M.driven); print(f"tonic floor: {len(_fl)} rows, {len(_flc)} cells")
...
pace = Pace(leg_stand=leg_stand, k=args.pace_k) if args.pace == "fixed" else RunningPace(); hers = HerSteering(rng); songdet = SongDetector()
m.v = 0.3; her.v = 0.15
