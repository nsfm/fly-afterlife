...
steer = Steering(gain=args.gain, rest_net=rest_net, leg_gain=leg_gain) if args.steer == "fixed" else (RunningBaselineSteering(gain=(args.gain if args.wheel == "DNa02" else (args.wheel_gain if args.wheel_gain is not None else -0.3)), leg_gain=leg_gain, wheel=args.wheel) if args.dn_gain == 0 else MultiWheelSteering(wheels=[("DNa02", args.gain), ("DN", args.dn_gain)], leg_gain=leg_gain))
