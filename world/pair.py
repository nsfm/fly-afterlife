...
PFL3V_NULL = None
if args.goal_wheel_v > 0 and GOAL is not None:
...
if args.world == "garden":
    ep.save(args.out, posts=np.array([[gx, gy, gr, ga] for gx, gy, gr, ga, gh in room.grass], np.float32), walls=dict(half=room.half, height=room.height, albedo=room.albedo), her_albedo=HER_ALB, body_r=BODY, her_r=HER_R,
            extra=dict(world="garden", floor_rgb=room.rgb, floor_half=room.half, grass=np.array(room.grass, np.float32), leaves=np.array(room.leaves, np.float32), stone=np.array(room.stone, np.float32), fruit=np.array(room.fruit, np.float32), puddle=np.array(room.puddle, np.float32), sunspot=np.array(room.sunspot, np.float32), wind=np.array(room.wind, np.float32), sun_dir=np.array(room.sun_dir, np.float32), **({"compass_wedges": np.array(_COMPASS["rows"], np.float32), "compass_pfl": np.array(_COMPASS["pfl"], np.float32), "compass_goal": np.array(_COMPASS["goal"], np.float32), "compass_switches": np.int32(_COMPASS["switches"]), "compass_sun_az": np.float32(RING["sun_az"])} if _COMPASS else {})))
