...
if args.thermo == "field": room.thermal = dict(base=25.0, x=1.5, y=1.5, dT=8.0, sigma=0.8)
...
from fly_afterlife.effectors import Steering, RunningBaselineSteering, Pace, RunningPace, HerSteering, SongDetector, dna02_rest_offset, standing_baselines, reflex_gain
...
leg_gain, asym_side = reflex_gain(M, RM, TACT_M, CH, SPF, kernel=(None if args.bristle == "hold" else Adapting()), fps=fps); print(f"touch reflex: leg-MN asymmetry (R-L)/(R+L) with left bristles {asym_side['L']:+.3f}, right {asym_side['R']:+.3f} -> gain {leg_gain:.0f} deg per unit asymmetry")
steer = Steering(gain=args.gain, rest_net=rest_net, leg_gain=leg_gain) if args.steer == "fixed" else RunningBaselineSteering(gain=args.gain, leg_gain=leg_gain)
