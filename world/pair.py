...
if args.goal is not None:
    ...
    GOAL = dict(cells=_fc2, stim=_stim.astype(np.float32), delta=_delta)
...
steer = LegSteering(LegModel(M), wheel_gain=args.gain, leg_gain=leg_gain) if args.effector == "legs" else Steering(gain=args.gain, rest_net=rest_net, leg_gain=leg_gain) if args.steer == "fixed" else (RunningBaselineSteering(gain=WGAIN, leg_gain=leg_gain, wheel=args.wheel, ema_n=args.steer_ema) if (args.dn_gain == 0 and args.goal_wheel == 0) else MultiWheelSteering(wheels=[(args.wheel, WGAIN)] + ([("DN", args.dn_gain)] if args.dn_gain > 0 else []) + ([("PFL3", args.goal_wheel, "fixed", args.goal_ema)] if args.goal_wheel > 0 else []), leg_gain=leg_gain, ema_n=args.steer_ema))
