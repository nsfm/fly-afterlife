...
fe_uv = FlyvisFrontEnd(args.model, geom="seam/eye_geom.npz", fps=fps, chunk=CH, deterministic=args.deterministic, types=["Mi15"]) if args.uv else None
# ---- his brain
...
if args.mirror != "off":
    from fly_afterlife.wiring import mirror_normalise; print("mirror normalisation:", mirror_normalise(M, scope=args.mirror))
groups = fe.groups(M)
groups_uv = fe_uv.groups(M, columns="seam/columns_all.npz", types=["Mi15"]) if fe_uv is not None else {}; groups_all = {**groups, **groups_uv}
if groups_uv: print("UV retina -> Mi15:", {k: len(v[0]) for k, v in groups_uv.items()})
RM = {}
_lm = np.load("world/legmn.npz"); LEGMN_SEL = (M.sc == "vnc_motor") if args.legmn == "all" else np.isin(np.arange(M.N), _lm["leg"])
for name, sel in [("DNa02", mty == "DNa02"), ("DNa01", mty == "DNa01"), ("DNb06", mty == "DNb06"), ("DNg13", mty == "DNg13"), ("IN12B014", mty == "IN12B014"), ("HS", np.char.startswith(mty, "HS")), ("pC1", np.char.startswith(mty, "pC1")), ("pIP10", mty == "pIP10"), ("mAL", np.char.startswith(mty, "mAL")), ("LC10a", mty == "LC10a"), ("ORN_VA1v", mty == "ORN_VA1v"), ("legMN", LEGMN_SEL), ("DN", M.sc == "descending_neuron")]:
...
for (t, s), (idx, hx) in groups_all.items():
    REG.add(ReceptorClass((f"Mi15_{s}" if t == "Mi15" else f"T4T5_{t}_{s}"), idx, Scaled(args.drive_gain), (lambda st, t=t, s=s, hx=hx: (st["a"][(s, t)][st["f"]] - st["rest"][(s, t)])[hx]), source="seam v2"))
if args.proprio > 0:
...
TONIC = (lambda t_: apply_tonic(REG, M, t_, 1.0 / fps, skip=TONIC_SKIP + ("Mi15_",)))   # the standing brain: floor, command, thermal cells at 25 C (09-18)
def TONIC_STAND(t_):   # standing = the floor without the walking command (10:31: with it, the pace reference rose with the dose; 10:55: the cells kept their drive when the row was skipped)
    apply_tonic(REG, M, t_, 1.0 / fps, skip=TONIC_SKIP + ("walk_", "Mi15_"))
...
steer = LegSteering(LegModel(M), wheel_gain=args.gain, leg_gain=leg_gain) if args.effector == "legs" else Steering(gain=args.gain, rest_net=rest_net, leg_gain=leg_gain) if args.steer == "fixed" else (RunningBaselineSteering(gain=WGAIN, leg_gain=leg_gain, wheel=args.wheel, ema_n=args.steer_ema) if args.dn_gain == 0 else MultiWheelSteering(wheels=[(args.wheel, WGAIN), ("DN", args.dn_gain)], leg_gain=leg_gain, ema_n=args.steer_ema))
