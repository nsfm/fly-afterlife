...
steer = LegSteering(LegModel(M), wheel_gain=args.gain, leg_gain=leg_gain) if args.effector == "legs" else Steering(gain=args.gain, rest_net=rest_net, leg_gain=leg_gain) if args.steer == "fixed" else (RunningBaselineSteering(gain=WGAIN, leg_gain=leg_gain, wheel=args.wheel) if args.dn_gain == 0 else MultiWheelSteering(wheels=[(args.wheel, WGAIN), ("DN", args.dn_gain)], leg_gain=leg_gain))
