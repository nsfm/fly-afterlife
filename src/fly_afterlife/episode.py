...
class Episode:
    def __init__(self, fps, chunk, eye, room, him, her, brains, readouts, registries, front_end, rest, effectors, lam=0.8, log_every=50):
    ...
    def render_and_move(self):
        ...
        for f in range(CH):
            lum[f] = self.eye.render(self.room.scene([her]), pos=(m.x, m.y, 0.5), heading_deg=m.h); self.POSE.append((m.x, m.y, m.h)); self.POSE2.append((her.x, her.y, her.h))
    ...
    def run(self, seconds: float):
        ...
        for c in range(T // self.CH):
            ...
            self.smells()
            singing = bool(self.songdet.hist) and len(self.songdet.hist) >= 5 and (log["song"] and log["song"][-1]) and dist < 0.4
            ...
            if self.female: her.h += self.hers.step(cntF, touched_f)
            song = self.songdet.step(cntM["pIP10"]); log["song"].append(song and self.female and dist < 0.4); log["dist"].append(dist); log["v_m"].append(m.v); log["v_f"].append(her.v)
            for k in self.RM: log[f"m_{k}"].append(cntM[k])
            for k in self.RF: log[f"f_{k}"].append(cntF[k])
            if c % self.log_every == self.log_every - 1:
    ...
    def save(self, path: str, posts, walls: dict, her_albedo: float, body_r: float, her_r: float, fov: float = 150.0):
        CH, eye, log, POSE = self.CH, self.eye, self.log, self.POSE; Tn = len(POSE)
        np.savez_compressed(path, fps=self.fps, chunk=CH, lum=np.concatenate(self.LUM), pose=np.array(POSE, np.float32), pose2=np.array(self.POSE2, np.float32), objects=posts, pillars=True, sky=self.room.sky, ground=self.room.ground, her_albedo=her_albedo, walls=np.array([walls['half'], walls['height'], walls['albedo']], np.float32), fov=fov, contacts=self.room.contacts,
                            ...
                            touch=np.array([{"L": 1, "R": 2, "B": 3}.get(t_, 0) for t_ in self.TOUCH], np.int8), touch_kind=np.array(self.TKIND, np.int8), body_r=body_r, her_r=her_r, heading_chunk=np.array([p[2] for p in POSE[::CH]]),
