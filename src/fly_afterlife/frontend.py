...
class FlyvisFrontEnd:
    def __init__(self, model="flow/0000/000", geom="seam/eye_geom.npz", fps=100, chunk=10, deterministic=False, root="/home/nate/code/fly-afterlife/flyvis_data"):
        ...
        ntype = self.net.connectome.nodes.type[:].astype(str); self.tix = {t: np.flatnonzero(ntype == t) for t in TYPES}
    ...
    def chunk(self, lum_chunk: np.ndarray) -> dict:
        ...
        for s in "LR":
            ...
            for t in TYPES: out[(s, t)] = act[:, self.tix[t]]
        return out
    ...
    def groups(self, brain, columns="seam/t4t5_columns.npz") -> dict:
