...
T4c image-up (measured, flyvis_dirs.py). our T4a is wired for front-to-back, and our
dorsal is known from the DRA. so: our sheet +sx (front) -> flyvis image-left, our
+sy (dorsal) -> flyvis image-up. flyvis BoxEye places hex (u, v) at image
...
ap.add_argument("--fps", type=int, default=100); ap.add_argument("--seconds", type=float, default=2.0)
args = ap.parse_args()
T = int(args.fps * args.seconds); eye = Eye(); g = np.load("seam/eye_geom.npz")

def ball_path(az_deg, t):
    """dark ball, radius 0.15, approaching along a bearing; distance 3 m -> 0.3 m over the last second."""
    az = np.radians(az_deg); dist = 3.0 if t < 1.0 else 3.0 - 2.7 * (t - 1.0)
    return (np.array([np.cos(az) * dist, np.sin(az) * dist, 0.5]), 0.15, 0.05)
def frame_scene(t):
    s = STIM
    ...
    if s == "recede_ahead":
        dist = 0.3 if t < 1.0 else 0.3 + 2.7 * (t - 1.0)
        return Scene(spheres=[(np.array([dist, 0, 0.5]), 0.15, 0.05)]), 0.0
    if s == "static_ahead": return Scene(spheres=[(np.array([1.0, 0, 0.5]), 0.15, 0.05)]), 0.0
...
for s in "LR":
    ...
    v = np.rint(-g["sx"][k]).astype(int); u = np.rint(-g["sy"][k] - v / 2.0).astype(int)
